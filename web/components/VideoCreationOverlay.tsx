...
import { VideoCreationWizard } from "./video-creation/VideoCreationWizard";
...
export function VideoCreationOverlay({
  ...
  const handleWizardComplete = (videoId: string) => {
    // In real implementation, this would navigate to the editor with the video
  ...
  return (
    ...
    >
      ...
      {/* Close button - high z-index to always be clickable */}
      <button
        onClick={onClose}
        className="absolute top-6 right-6 z-[100] p-3 rounded-full bg-neutral-900/80 border border-neutral-700 hover:border-orange-500 hover:bg-neutral-800 transition-all duration-300 group"
      >
        <X className="w-6 h-6 text-neutral-400 group-hover:text-orange-500 transition-colors" />
      </button>

      {/* Main content */}
      <div className="relative z-10 w-full h-full flex items-center justify-center px-8 py-16">
        {wizardStarted ? (
          <VideoCreationWizard
            projectId={projectId}
          ...
          <div className="flex flex-col items-center gap-12">
            ...
            >
              <button
                ...
                className="group relative px-12 py-4 bg-gradient-to-r from-orange-500 to-orange-600 rounded-lg font-bold uppercase tracking-widest text-sm text-white hover:from-orange-400 hover:to-orange-500 transition-all duration-300 shadow-lg shadow-orange-500/25 hover:shadow-orange-500/40"
              >
