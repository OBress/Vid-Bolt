  ...
  const handleBeatEdit = useCallback(
    (beatIndex: number, newNarration: string) => {
      ...
      const newScript = output.expandedBeats
        .map((b) => (b.beatIndex === beatIndex ? newNarration : b.narration))
        .join("\n\n");
  ...
  return (
    <div className="flex h-[calc(100vh-160px)] gap-4 w-full max-w-[98vw] mx-auto px-6 py-6">
      ...
      <div className="w-72 shrink-0 flex flex-col gap-4 h-full">
          ...
          <div className="flex-1 overflow-y-auto space-y-2 mb-3 min-h-0">
            {chatMessages.length === 0 ? (
              <div className="text-xs text-neutral-500 text-center py-4">
                Highlight text in the script, click + to add context, then ask
                AI to rewrite.
              </div>
            ) : (
        ...
        {/* Actions */}
        <div className="bg-neutral-900/50 border border-neutral-800 rounded-xl p-4 space-y-2 shrink-0">
          <Button
            onClick={handleConfirm}
            className="w-full h-11 bg-gradient-to-r from-orange-500 to-orange-600 hover:from-orange-400 hover:to-orange-500 text-white font-bold uppercase tracking-widest gap-2 text-sm"
          >
          ...
          <Button
            onClick={onBack}
            variant="ghost"
            className="w-full text-neutral-500 hover:text-white gap-2 text-sm"
          >
            <ArrowLeft className="w-4 h-4" />
            Back
          </Button>
