...
export const useCaptions = () => {
  ...
  /**
   * Creates a caption clip from caption data and adds it to the timeline
   * Returns the clip ID for selection
   */
  const createCaptionOverlay = useCallback((captions: Caption[]): string => {
    if (captions.length === 0) {
      throw new Error('Cannot create clip with empty captions');
    }
    ...
    // Default caption positioning (centered at bottom third of composition)
    const captionWidth = Math.round(width * 0.65);
    const captionHeight = Math.round(height * 0.25);
    const captionX = Math.round((width - captionWidth) / 2);
    const captionY = Math.round(height * 0.62);
    ...
    const clipId = addClip({
      ...
      transform: {
        ...
        rotation: 0,
        opacity: 1,
        zIndex: 200, // Captions should be on top
      },
      data: {
        captions, // Store the captions data
      },
      styles: {
    ...
    return clipId || '';
  }, [addClip, selectClip, currentTime]);
