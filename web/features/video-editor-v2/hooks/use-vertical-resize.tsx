...
export const useVerticalResize = (options: UseVerticalResizeOptions = {}): UseVerticalResizeReturn => {
  ...
  // Only clamps to min/max on initial load, not on every maxHeight change
  const getSavedHeight = useCallback(() => {
      ...
      try {
        ...
        if (saved) {
          ...
          if (!isNaN(height) && height >= minHeight) {
            // Return saved height, only clamping to absolute bounds
    ...
    // Ensure initial height is within bounds
  ...
  // Note: We intentionally do NOT clamp height when maxHeight changes
  // This preserves user's manual resize preference
  // The height is only clamped during active resize operations
