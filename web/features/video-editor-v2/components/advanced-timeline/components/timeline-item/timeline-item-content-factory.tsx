...
export interface BaseItemContentProps {
  ...
  isSelected?: boolean; // Add selected state to base interface
 ...
 * 
 * Wrapped in memo to prevent unnecessary re-renders when props haven't changed.
 */
export const TimelineItemContentFactory: React.FC<TimelineItemContentFactoryProps> = memo(({
  type,
  label,
  data,
  itemWidth,
  ...
  const commonProps = {
    ...
    isSelected, // Pass selected state to all components
  ...
  switch (type) {
    ...
    case TrackItemType.MOTION_GRAPHICS:
      return <MotionGraphicsItemContent {...commonProps} />;
