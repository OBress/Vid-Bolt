...
interface VideoItemContentProps {
  ...
  data?: {
    ...
    intervalSec?: number;
    mediaStart?: number;
    content?: string; // Instant thumbnail URL from upload
  };
  itemWidth: number;
  itemHeight: number;
  start: number;
...
export const VideoItemContent: React.FC<VideoItemContentProps> = ({
  ...
  const instantThumbnail = data?.content; // Stored thumbnail from upload
  ...
  useEffect(() => {
    ...
    if (isLoadingThumbnails && !showLoading) {
      // If we're loading, but we have old thumbnails or instant thumbnail, keep showing them
      shouldShowThumbnails = !!(spriteUrl && rectForTime) || !!instantThumbnail;
    } else {
      shouldShowThumbnails = isShowingThumbnails || (!!instantThumbnail && isLoadingThumbnails);
    }
    
    onThumbnailDisplayChange?.(shouldShowThumbnails);
  }, [isShowingThumbnails, isLoadingThumbnails, showLoading, spriteUrl, rectForTime, instantThumbnail, onThumbnailDisplayChange]);
  ...
  // Show instant thumbnail while loading sprite sheet (uses stored thumbnail from upload)
  // This provides immediate visual feedback without waiting for sprite generation
  if (instantThumbnail && isLoadingThumbnails) {
    return (
      <div className="flex items-center h-full w-full overflow-hidden relative rounded-[3px]">
        <div 
          className="w-full h-full bg-cover bg-center opacity-90"
          style={{
            backgroundImage: `url(${instantThumbnail})`,
  ...
  // Show loading spinner only if no instant thumbnail and loading for more than 1 second
  if (showLoading && !instantThumbnail) {
    return (
      <TimelineItemLabel
        icon={Loader2}
        label={"Loading..."}
