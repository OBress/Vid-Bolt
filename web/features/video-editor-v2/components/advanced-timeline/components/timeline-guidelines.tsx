...
export const TimelineGuidelines: React.FC<TimelineGuidelinesProps> = ({
  ...
  // Get snapping state from store - only show guidelines when snapping is enabled
  const snappingEnabled = useVideoEditorStore(state => state.snappingEnabled);
  
  // Calculate alignment guidelines based on current drag position
  const guidelines = useMemo((): Guideline[] => {
    // Don't show guidelines if snapping is disabled
    if (!snappingEnabled || !isDragging || !currentDragPosition || !draggedItemId) {
      return [];
    }
    ...
    return guidelines;
  }, [tracks, isDragging, currentDragPosition, draggedItemId, totalDuration, snappingEnabled]);
