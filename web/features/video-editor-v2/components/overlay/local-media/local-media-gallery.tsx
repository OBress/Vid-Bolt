...
export function LocalMediaGallery({
  ...
  const handleDragStart = useCallback((file: any) => (e: React.DragEvent) => {
    ...
    startMediaDrag(
      timelineType as 'video' | 'image' | 'audio',
      mediaSrc,
      {
        duration,
        name: file.name,
  ...
  const handleDragEnd = useCallback(() => {
    // Clear unified store drag state
    endDrag();
  }, []);
  ...
  const renderMediaItem = useCallback((file: any) => {
    ...
    const getThumbnailUrl = () => {
      if (file.type === "image") {
        // For images, use the full image as thumbnail for best quality
        if (file.path.startsWith('blob:') || file.path.startsWith('http')) {
          return file.path;
        }
        return `/api/latest/local-media/serve/${file.path.startsWith('/') ? file.path.substring(1) : file.path}`;
      }
        ...
        <div className="aspect-video relative bg-muted/50 overflow-hidden">
          ...
          {file.type === "video" && (
            <>
              ...
              <div className="absolute bottom-1.5 right-1.5 bg-black/80 text-white text-[10px] font-medium px-1.5 py-0.5 rounded shadow-sm">
  ...
  return (
    <div className="h-full flex flex-col">
      ...
      <div className="flex-1 flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto sidepanel-scrollbar">
          {isLoading ? (
