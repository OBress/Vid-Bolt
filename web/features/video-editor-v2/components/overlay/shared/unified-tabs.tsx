...
interface FlexibleTabsProps {
  ...
  className?: string;
  /** Additional CSS classes for tab content containers */
...
export const UnifiedTabs: React.FC<UnifiedTabsProps> = (props) => {
        ...
        <Tabs value={activeTab} onValueChange={onTabChange}>
          <TabsList>
            ...
            {showAllTab && (
              <TabsTrigger
                value="all"
                className="text-xs font-extralight border-b-2 border-transparent rounded-none data-[state=active]:bg-primary data-[state=active]:text-primary-foreground"
              >
                All ({totalCount})
              </TabsTrigger>
            )}
            ...
            {sourceResults.map((source) => (
              <TabsTrigger
                ...
                className="text-xs font-extralight border-b-2 border-transparent rounded-none data-[state=active]:bg-primary data-[state=active]:text-primary-foreground"
              >
                <span>{source.adaptorDisplayName}</span>
                <span className="text-[10px] opacity-70">({source.itemCount})</span>
  ...
  const contentClassName = ('contentClassName' in props ? props.contentClassName : "") || "";
  const isControlled = 'activeTab' in props && 'onTabChange' in props;

  const getTabTriggerClassName = (tab: TabItem) => {
    const baseClasses = "text-xs font-extralight border-b-2 border-transparent rounded-none data-[state=active]:bg-primary data-[state=active]:text-primary-foreground";
    ...
    <Tabs {...tabsProps} className={`w-full ${className}`}>
      ...
      {tabsData.some(tab => tab.content) && tabsData.map((tab) => (
        <TabsContent 
          key={tab.value} 
          value={tab.value} 
          className={`space-y-4 mt-2 h-auto focus-visible:outline-none ${contentClassName}`}
        >
          {tab.content}
        </TabsContent>
      ))}
