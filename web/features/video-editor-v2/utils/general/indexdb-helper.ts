...
/**
 * Clear ALL autosave records from the database
 * @returns Promise that resolves when the clear is complete
 */
export const clearAllAutosaves = async (): Promise<void> => {
  try {
    const db = await initDatabase();
    const transaction = db.transaction([AUTOSAVE_STORE], "readwrite");
    const store = transaction.objectStore(AUTOSAVE_STORE);
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onsuccess = () => {
        console.log('[IndexedDB] All autosaves cleared');
        resolve();
      };

      request.onerror = (event) => {
        console.error("Error clearing all autosaves:", event);
        reject("Error clearing all autosaves");
      };
...
if (typeof window !== 'undefined') {
  (window as any).clearVideoEditorDB = clearDatabase;
  (window as any).clearAllAutosaves = clearAllAutosaves;
}
